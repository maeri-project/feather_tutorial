...
if [ "$ARCH" = "x86_64" ]; then
    IMAGE_NAME="raic:latest-amd64"
elif [ "$ARCH" = "arm64" ] || [ "$ARCH" = "aarch64" ]; then
    IMAGE_NAME="raic:latest-arm64"
else
...
EPHEMERAL_NAME="raic-rm-${USER_NAME}"
DEFAULT_PERSISTENT_NAME="raic-default-${USER_NAME}"
...
# Helper: List all persistent containers of form raic-<base>-<username>
list_persistent_all() {
    docker ps -a --format '{{.Names}}\t{{.Image}}\t{{.Status}}' \
      | awk -v user="${USER_NAME}" -F '\t' '$1 ~ ("^raic-.*-"user"$") { print $0 }'
}

# Helper: Ensure name is normalized to: raic-<base>-<username>
normalize_name() {
    local raw="$1"
    local prefix="raic-"
