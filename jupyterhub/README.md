...
Docker-based JupyterHub for the RAIC tutorial, running on `jupyterhub.example.com`.
...
JupyterHub will be available at `https://jupyterhub.example.com`.
...
docker run --rm -p 80:80 \
  ...
  "pip install --upgrade certbot pyOpenSSL cryptography && \
   certbot certonly --standalone -d jupyterhub.example.com \
