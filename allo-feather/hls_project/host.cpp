...
int main(int argc, char** argv) {
    ...
    std::vector<uint8_t>  source_in1(512);
    ifile1.read(reinterpret_cast<char*>(source_in1.data()), 512);
    ...
    std::vector<uint8_t>  source_in2(24);
    ifile2.read(reinterpret_cast<char*>(source_in2.data()), 24);
    std::ifstream ifile3("input3.data", std::ios::binary);
    if (!ifile3.is_open()) {
      std::cerr << "Error: Could not open input file.\n";
      return 1;
    }
    ...
    std::vector<uint8_t>  source_in3(64);
    ifile3.read(reinterpret_cast<char*>(source_in3.data()), 64);
    ...
    OCL_CHECK(err, cl::Buffer buffer_in1(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 512, source_in1.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_in2(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 24, source_in2.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_in3(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, 64, source_in3.data(), &err));
    ...
    {
        std::ofstream ofile0;
        ofile0.open("output0.data", std::ios::binary);
        if (!ofile0) {
            std::cerr << "Failed to open output file 0!" << std::endl;
            return EXIT_FAILURE;
        }
        ofile0.write(reinterpret_cast<const char*>(source_in3.data()), 64);
        ofile0.close();
    }
